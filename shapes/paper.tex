...
\begin{abstract}
We measure the shape and alignment of the stellar and dark matter mass distribution in 11 strong-lens galaxies. We show that the projected dark matter density profile in these systems -- under the assumption of a Chabrier stellar initial mass function -- shows significant variation from galaxy to galaxy. Those with an outermost image beyond $\sim 10$\,kpc are very well fit by a projected NFW profile; those with images within 10\,kpc appear to be more concentrated than NFW, as expected if their dark halos contract due to baryonic cooling. We find that over several half-light radii, the dark matter haloes of these lenses are rounder than their stellar mass distributions. While the haloes are never more elliptical than $s_{dm} = 0.3$ (where $s$ is defined such that $s=0$ is spherical and $s=1$ is infinitely flattened), their stars can extend to $s_* > 0.3$. Galaxies with high dark matter ellipticity and weak external shear show strong alignment between light and dark; those with strong shear ($\gamma \gtrsim 0.1$) can be highly misaligned. This is reassuring since isolated misaligned galaxies are expected to be unstable. Our results provide a new constraint on galaxy formation models. For a given cosmology, these must explain the origin of both very round dark matter haloes and misaligned strong-lens systems.
{\bf JR. I hope you agree with my analysis of the radial profiles. I've spotted a very cute little result here already and thought we should highlight it. I discuss it further in the results and conclusions. Also, we now define $s$ to be the `ellipticity'. But later on when discussing weak lensing we define a different ellipticity parameter $e$. The referee took us to task on this also I think. Are there really two different definitions of ellipticity in the literature? If so, perhaps we should clarify this up-front when we define $s$. Alternatively, if $s$ is unique to our paper then why not just change $s \rightarrow e$ throughout? Or we could call our $s$ parameter the `shape parameter' and $e$ the `ellipticity'. I have no strong view on which is best, but it's a bit confusing as it is right now}.
\end{abstract}
...
\end{equation}
where $\lambda_{1}$ and $\lambda_{2}$ ($\lambda_{1} \geq \lambda_{2}$) are the eigenvalues. We use this ellipticity estimator to quantify the shapes both of the stellar-mass distribution $s_*$ and of the dark-matter distribution $s_{dm}$. By default, we use the dark and stellar mass distributions out to the outermost lensing image. In Figure \ref{fig:wedgesradii}, we explore the effect of averaging instead over multiples of the half-light radius $R_e$. The mass pixels used in this work are larger than image pixels, but at most 4\% of the diameter of the the full mass map. The central region has smaller pixels, to allow for a density cusp at the centre.

%For the dark matter, only the mass distribution out to the radius of the outermost image is used; for the stars, the distribution with within different multiples of $R_e$ (defined as the half-light radius measured with respect to the Petrosian flux) is used. It is necessary to have a cutoff for the regions used to compute the inertia tensor, or else it would be dominated by the outer regions where there is little information \textbf{[CLAUDIO: Actually, I estimate the shapes for the luminous and dark matter in the same way. I only consider different Petrosian radii in Figure 3, setting different multiples of $R_e$ as a limit for both distributions. Figure 3 gives a sense of how much the estimates change if the distributions are analyzed only within a certain region...]}.
...
Each model ensemble consists of $10^4$ models. We apply our shape and alignment estimators to each model, in this way obtaining the median and 68\% confidence intervals of $s$ and $\Delta \theta$, for each lens.
...
\begin{table*}
  \begin{center}
    \begin{tabular}{l l l l l l l l l l l l l}
      % \multirow{2}{*}{Lens} & \multirow{2}{*}{$s_{*}$} & \multirow{2}{*}{$s_{dm}$} & \multirow{2}{*}{$\Delta\theta$ [$^{\circ}$]} & \multirow{2}{*}{$e_{*}$} & \multirow{2}{*}{$e_{dm}$} & \multirow{2}{*}{$e_{dm}/e_{*}$} & \multirow{2}{*}{$f_h$} & \multirow{2}{*}{$|\gamma|$} & \multirow{2}{*}{$\theta_{g}$ [$^{\circ}$]} & $R_{vir}$ & $M_{vir}$ & \multirow{2}{*}{Env} \\
      %  & & & & & & & & & & [kpc] & $[10^{12} \ M_{\odot}]$ & \\ \hline
      % 0047 & 0.27 & 0.04$\pm$0.02 & -41$\pm$33 & 0.15 & 0.02$\pm$0.01 & 0.13$\pm$0.07 & 0.02$\pm$0.15 & 0.07$\pm$0.01 & 120$\pm$5 & 360.7  & 10.0 & G(9) \\
      % 0414 & 0.19 & 0.06$\pm$0.03 & 50$\pm$15 & 0.10 & 0.03$\pm$0.01 & 0.30$\pm$0.14 & -0.05$\pm$0.15 & 0.17$\pm$0.02 & 166$\pm$4 & 256.9  & 12.4 & ... \\
      % 0712 & 0.43 & 0.08$\pm$0.03 & -0$\pm$10 & 0.28 & 0.04$\pm$0.01 & 0.15$\pm$0.05 & 0.15$\pm$0.05 & 0.17$\pm$0.01 & 141$\pm$3 & 446.5  & 15.5 & ... \\
      % 0911 & 0.22 & 0.07$\pm$0.02 & 25$\pm$14 & 0.12 & 0.04$\pm$0.01 & 0.29$\pm$0.10 & 0.19$\pm$0.12 & 0.28$\pm$0.02 & 99$\pm$2 & 369.1  & 22.7 & C \\
      % 0957 & 0.19 & 0.10$\pm$0.03 & 4$\pm$11 & 0.10 & 0.05$\pm$0.01 & 0.49$\pm$0.14 & 0.49$\pm$0.14 & 0.09$\pm$0.03 & 159$\pm$8 & 1053.1 & 175.2 & C \\
      % 1115 & 0.30 & 0.11$\pm$0.02 & -75$\pm$7 & 0.18 & 0.06$\pm$0.01 & 0.32$\pm$0.07 & -0.28$\pm$0.08 & 0.11$\pm$0.01 & 154$\pm$3 & 616.8  & 31.1 & G(13) \\
      % 1422 & 0.05 & 0.06$\pm$0.03 & -51$\pm$18 & 0.03 & 0.03$\pm$0.01 & 1.03$\pm$0.51 & -0.23$\pm$0.65 & 0.26$\pm$0.01 & 39$\pm$1 & 450.8  & 13.0 & G(17) \\
      % 1608 & 0.47 & 0.28$\pm$0.01 & -12$\pm$1 & 0.31 & 0.16$\pm$0.01 & 0.53$\pm$0.03 & 0.48$\pm$0.03 & 0.19$\pm$0.01 & 145$\pm$2 & 317.1  & 10.0 & G(8) \\
      % 2016 & 0.47 & 0.12$\pm$0.02 & 20$\pm$6 & 0.30 & 0.06$\pm$0.01 & 0.21$\pm$0.04 & 0.16$\pm$0.04 & 0.14$\pm$0.02 & 42$\pm$4 & 299.3  & 22.1 & C(69) \\
      % 2045 & 0.17 & 0.03$\pm$0.02 & -4$\pm$20 & 0.09 & 0.02$\pm$0.01 & 0.19$\pm$0.11 & 0.19$\pm$0.11 & 0.11$\pm$0.02 & 23$\pm$7 & 562.8  & 104.0 & ... \\
      % 2237 & 0.24 & 0.05$\pm$0.03 & -7$\pm$19 & 0.13 & 0.03$\pm$0.01 & 0.21$\pm$0.10 & 0.20$\pm$0.11 & 0.06$\pm$0.01 & 159$\pm$4 & 1961.0 & 485.3 & ... \\
...
Before discussing the shapes and alignments, we first measure the dark matter profiles of the strong lens galaxies, comparing these with projected Navarro-Frenk-White profiles \citep[NFW;][]{1996ApJ...462..563N} truncated at the virial radius $R_{vir}$ \citep{2011ApJ...740..102K}:
\begin{equation}\label{eq:nfw}
   \frac{\rho(R)}{\rho_{crit}} = \frac{\delta_{c}}{(cR/R_{vir})(1+cR/R_{vir})},
\end{equation}
where $\rho_{crit}$ and $\delta_{c}$ are parameters set by the cosmology considered, $R$ is the radius from the centre, and $c$ is the concentration parameter. The concentration is well-correlated with $M_{vir}$ with some redshift dependence \citep{2011ApJ...740..102K}: 
\begin{equation}\label{eq:concentration}
 ...
 \end{split}
\end{equation}
where $c_{0}(z)$ and $M_{0}(z)$ are fitting parameters (see Table 3 of \citet{2011ApJ...740..102K}). For each lens galaxy we choose the parameters at the redshift closest to the lens redshift. {\bf JR. If this is projected mass, then wouldn't it be better to use $\Sigma$ rather than $\rho$. Also, we should explain that $\rho_{\rm crit}(z)$ is the critical density of the universe, not to be confused with the critical lensing surface density. It is also a function of redshift (I presume we change this also with the lens redshift). We should also define $\delta_c$. Finally, I would recommend Awriting down the relationship between $\rho(R) \equiv \Sigma(R)$ and $\langle \kappa(R)\rangle$.}
...
Our lens sample is small -- just 11 strong lensing systems -- yet it is interesting that we find no lens with strong misalignment and weak external shear. This is perhaps to be expected. \citet{XXX} show that YYY. 

\textbf{JR. I think comparisons with weak lensing have a section of their own and don't need to go in the conclusions. I moved them to this discussion section, though, to ensure that they stand out. I added a new subsection in discussion also about misalignments and stability (above). This should deal with that issue.}
...
% AND :: 1979ApJ...233..872H,1988A&A...206..269M,2007ApJ...670.1027A,2015arXiv150203429D
...
\item The dark matter haloes of all lenses in our sample are rounder than the light distribution over the range $1R_e < R < 5R_e$. As we average over larger radii, the lenses, except the ones with a large stellar ellipticity, become increasingly elliptical in their stellar distributions. The same trend can be seen also for the dark matter, though the effect is not statistically significant. The dark matter haloes are never more elliptical than $s_{dm} = 0.2$, while their stars can extend to $s_* > 0.3$.

\item Three systems have a high stellar ellipticity ($s_* > 0.45$) and correspondingly high alignment between light and dark. One of these -- {\it1608} -- is a known merging pair. We speculate that the other two lens systems ({\it0712} and {\it2016}) may also be recent post-merger systems. 
