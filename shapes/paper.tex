...
mass models given a set of basic assumptions, or priors.  {\bf Hence, \Glass\ does not yield the most probable mass model. Rather, we can describe the distribution of all the allowed models. (CB: I'VE ADDED THIS SENTENCE AS I FELT SOMETHING LIKE THAT MIGHT BE NEEDED AT THIS POINT. IT PROBABLY NEEDS TO BE IMPROVED THOUGH)} In this context,
...
As it is shown in \citep{2014arXiv1401.7990C}, the recovery of the shape information of the lens is only good if there are at least four images. Therefore, we only consider the quad lenses and the twin double \textit{Q0957+561} out of the sample presented in \citep{2011ApJ...740...97L}. Thus, the sample consists in total of 11 lenses. Each individual system is presented below.
...
\textit{B2045+265} (hereafter \textit{2045}) is probably an elliptical galaxy \citep{2007MNRAS.378..109M}. It was initially classified as a late-type Sa galaxy \citep{1999AJ....117..658F}, the velocity dispersion however seems too high. As the source redshift is rather low, a large lens mass is required. \cite{2007MNRAS.378..109M} therefore conclude that it is more likely an elliptical galaxy. To the west of the lens, a group was found at a similar redshift as the lens \citep{1999AJ....117..658F}. There is furthermore evidence for a dwarf satellite galaxy \citep{2007MNRAS.378..109M}. As the measurements are inconclusive, we choose to not include this satellite.

\textit{Q2237+030} (hereafter \textit{2237}) is a barred spiral \citep{1988AJ.....95.1331Y}, the only non-early type lens in this sample. With only a redshift of $z=0.04$, it is the closest lens of the sample. Due to the low redshift, the probed physical scales are small. Therefore, the bulge component of the galaxy dominates the lens, what makes the stellar population synthesis modelling possible. No further inquiries into the environment were made. It is

Further information on the sample can be found in \citep{2011ApJ...740...97L,2012A&A...538A..99S}. The most important quantities of each system are given in Table~\ref{tab:lensproperties}. Due to the different angular separations of the images, different scales of the lens galaxies are probed.
...
{\bf (CB: In general, do we need to link the results better to previous studies? We mention them already in the introduction and since they're not directly comparable, I'm not sure how much emphasis should be put on the comparison.)}

We have applied \Glass\ to the data set described in \S\ref{sec:data} consisting of 11 lenses. The reconstructed mass maps and the in this work used stellar mass maps \citep{2011ApJ...740...97L} are displayed in the Appendix.

Figure~\ref{fig:wedgesall} displays the shapes (see \S\ref{sec:shapemethod}) and their alignment of the dark and stellar mass distribution. In this Figure all the distribution up to the last-image radius $R_{L}$ are probed. Different features are evident. First, the reconstructed dark matter distribution of the considered lenses is rounder than the stellar distribution. Second, most lenses are in the bottom-left corner. Thus, the dark matter distributions are typically quite round, while the stellar distributions can be rather elliptical. {\bf HERE MAYBE SOME SENTENCES HOW THIS RESULT IS SURPRISING}

Third, 3 out of the 11 systems, {\it0712}, {\it2016}, and {\it1608}, stand out due to the larger stellar ellipticity ($s_* > 1.6$) and at the same time an alignment of the dark matter and stellar distributions. One of those lenses, {\it1608}, is an known merger of an early-type galaxy with a probably late-type galaxy \citep{2003ApJ...584..100S}. We suggest that the other two systems, {\it0712} and {\it2016}, may be post-merger systems as well and therefore display those similarities with {\it1608}.

Fourth, dark matter and stellar distributions with weak shear are aligned apart from {\it0047}, which is almost spherical ($s_{dm}\sim1.05$). We note that in {\it1115} the semi-major axes seems to be aligned with the semi-minor axes of the other component. Systems requiring a larger external shear, {\it0414}, {\it0911}, and {\it1422}, can display a certain misalignment between the distributions. The latter two systems are located in dense environments (see Table~\ref{tab:lensproperties}) that can cause those misalignments {\bf (CB: NEED SOME REFERENCES HERE)}. {\bf (CB: I'M NOT SURE WHETHER THE SHEAR FOR 0414 IS LARGE ENOUGH THAT IT COULD BE A PROBLEM THAT NOT ENVIRONMENT HAS BEEN DETECTED (THE SYSTEM IS ADDITIONALLY RATHER MISALIGNED))}

We performed tests for the robustness of the shape estimate and its dependence on the shear prior. Only two systems, {\it1422} and {\it2045}, reacted to a stricter prior on the external shear $|\gamma|$. {\it1422} requires a rather large external shear ($|\gamma|\sim0.25$). Restricting the range of the allowed external shear values did not affect the misalignment of the dark matter and stellar mass distributions. It however increased the ellipticity of the reconstructed mass distribution. We observed a trade-off between the galaxy's ellipticity and the magnitude of the external shear. {\it2045} on the other hand is particularly interesting. Allowing the shear to roam free as for the other lenses, favours an anomalously high shear ($\gamma > 0.5$), with strong misalignment. However, such models also produce spurious extra images along the arc. We find that limiting the maximum shear $|\gamma|$ to 0.1 removes the extra images, though the lens then pushes on its maximum shear prior. We find that the mass map is robust against a stronger shear prior, and it thus again does not affect the alignment of the distributions. It produces dark matter distributions well-aligned with the stars. We believe that a strong shear prior for this particular lens is justified by the non-observation of the spurious extra images; none of the other lenses in our sample have a shape profile that is sensitive to our shear prior.

Figure~\ref{fig:wedgesradii} is similar to Figure~\ref{fig:wedgesall} with the difference that shapes are averaged within different multiples of $R_e$ in contrast to the whole distribution within the outermost image. It looks in general similar to Figure~\ref{fig:wedgesall}. {\it0957} and to some degree {\it1422} display however some signs of isophotal twist (see also Appendix). Additionally, there seems to be a trend that the ellipticity of the stars and potentially of the dark matter, though the latter is consistent with constant ellipticity, increases with increasing radius. {\bf (CB: DO WE NEED TO ADD AN INTERPRETATION WHY THIS IS?)}
