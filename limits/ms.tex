...
multiple source redshifts; (iv) stellar kinematics provide a robust measure of the mass at the half light radius of the stars $r \gg r_{1/2}$ that can also break the steepness degeneracy if $r_{1/2} \neq r_E$ -- the Einstein radius; and (v) if $r_E \sim r_{1/2}$, then stellar kinematic data can be used to probe the stellar velocity anisotropy 
$\beta(r)$ -- an interesting quantity in its own right. Where
...
\end{equation}
where the second relation follows from the fact that the light deflection angle is maximised when the lens is equidistant between the observer and the source \citep[e.g.][]{1992grle.book.....S}.
...
\item In free-form models, there is no specified parametric form for
  ...
  \cite{2006MNRAS.367.1209L} implemented a particularly elegant prior,
  which is that the mass distribution must be non-negative and no
  extra images are produced.  Free-form models are more commonly used with
  ...
  the less restrictive assumptions of free-form models resolved those
...
\item Model ensembles, exploring a diverse range of possible mass
  ...
  combating the non-uniqueness of models.  Model ensembles are
...
In this paper, we use a new program---\Glass, the Gravitational Lensing AnalysiS Software---to determine what
...
if the $D_S/D_{LS} - \kappa(\vec\theta)$ \hilight{ $\leftarrow$ What is meant
by this? Can we just write $\tau(\vec\theta)$? }are each multiplied by an
arbitrary constant.  This amounts to multiplying the arrival-time surface by a
constant factor.  In fact there are infinitely many degeneracies
...
lensing); $G$ is Newton's gravitational constant; and $M(r)$ is
...
$M(r)$ and the velocity anisotropy $\beta(r)$. This can be understood
...
estimated if an independent measure of $M(r)$ is available -- for example
coming from strong lensing. Finally, we may integrate out the effect of unknown
$\beta(r)$ by performing a luminosity weighted average over all radii: 

\begin{equation}
\overline{\sigma} = \frac{\int_0^{a_p} I(R) \sigma_p^2(R) 2\pi R dR}{\int_0^{a_p} I(R) 2\pi R dR}
\label{eqn:virialaverage}
\end{equation} 
where $a_p$ is the `aperture' over which the integral is performed. Ideally we
should take $a_p \rightarrow \infty$, however this is not practical for real
data where the surface brightness of the stars falls off rapidly with $a_p$. In
practice, $a_p > 2r_{1/2}$, where $r_{1/2}$ is the half light radius of the
stars, appears to be sufficient (see \S\ref{sec:results}). The above average
produces a single robust measure of the mass $\overline{\sigma}$ that is
independent of $\beta(r)$, provided that the stellar system is in a dynamic
quasi-equilibrium that obeys the virial theorem
\citep{2009ApJ...704.1274W,2010MNRAS.406.1220W,2012ApJ...754L..39A}. (Such an
averaging procedure was first used in a combined lensing/stellar kinematic
analysis in \citealt{2002MNRAS.337L...6T}.)
...
XXX
\end{equation} 
\hilight{We are not rejecting any models. Shall we remove this section, or just rewrite it?}
\hilight{Typically, this results in a rejection rate of XXX\%}. 
...
as a robust measure of $M(r_{1/2})$ (see \S\ref{sec:kinematics}). 
...
are shown in \tabref{mock galaxy params}. \hilight{JR. Need to add explanation of
...
models. Image colors represent a common source. In the case of the extended
sources the redshift is at R1, while the redshift is varied between the systems
in the ZConstrast cases. Grey circles are a visual aid to help determine radial
separation between images. The axes are measure in arcseconds.}
...
have been seen \hilight{(CITES)}). We assumed for all our tests that the
...
profile. \hilight{JR.  What happens if we don't assume this symmetry? We ought
...
median.  The true density profile from the mock data is also plotted for
comparison. The vertical lines mark the radial position of the images. The
final plot on the right is of the enclosed mass. As expected, the error bars
are smallest in the region of the images where the most information about the
lens is present. The dip in the profile at the end of the profile is due to the
...
\caption{
    ...
    used in \Glass, the modelling favors steep solutions without additional
    information.
\textbf{Left:}
The ensemble average arrival time surface with just the isocontours for the
saddle points drawn. The central diamonds show the reconstructed source
position.
\textbf{Middle:}
The surface density of the dark matter (magenta), the stars (yellow), and the total (blue).
...
\textbf{Right:}
The cumulative mass. The error bars on all plots are $1\sigma$, but the grey area shows the full range of models for DM+Stars (blue).}
...
\caption{Our main results showing the quality of model recovery \eqnrefp{ferror R} for all our test cases. 
Within
...
indicate where the stellar mass has been included. Error bars show the $1\sigma$ interval of the model ensemble.}
\label{main results}
\end{figure*}
...
\caption{Similar to \figref{main results}, we show here the fractional error
for the pixel-wise recovery \eqnrefp{ferror theta} of all models. The colors and labels are the same
as before. Error bars show the $1\sigma$ interval of the model ensemble.}
...
increase (and for $f$ to decrease). By adding more measurable information to
each configuration the quality can also be affected. When both time delays and
a central image are present the quality is highest. A double is known to
...
arc from the extended source is sufficient to correct this. \hilight{Mention the saturation of the two panels.}

We can also directly measure the recoverability of the shape of the mass
distribution by computing the eigenvalues of the inertia tensor. We define our
shape by the ratio of these eigenvalues $s=\lambda_1/\lambda_2$, where the
$\lambda_1$ is always the largest value. Again, we only consider mass up to one
radial position passed the outermost image and compute our the fractional error
%
...
for each mock galaxy and each test case is shown in \figref{shape results}.

\begin{figure}
\plottwo{AAExtendedDoubleR1_TmS-shape-hist.pdf}{BCZContrastR1R3_TmS-shape-hist.pdf}
\caption{The two plots in the left most column show sample distributions of
  ...
  The upper (poor) plot is from the \mockAA{} galaxy, while the lower (good) plot is
  from the \mockBC{} galaxy. The black vertical lines mark the $2\sigma$
...
configuration, these two radii are well-separated. The actual variation in $\beta(r)$ is show also shown (\textbf{right}).}
...
\hilight{This is a bit overstated.}
With the stellar mass lower bound, the improvement of the reconstruction
quality shown in \figref{main results} is quite dramatic for the doubles in the
...
---where the stars contribute negligibly to the potential---are unaffected.
...
\hilight{This paragraph should be removed.}
...
is poorly constrained \hilight{(XXX)}, and a model where the mass profile is very well
constrained by the lens data \hilight{(YYY)}. In the former case, we use
...
models from the ensemble above. The results for $\beta$ are shown in the right panel of
\figref{fig:sigp}. 
...
robust measure of $M(r_{1/2})$ from the stellar kinematics. This latter is
robust to both uncertainties in $\beta(r)$ and to our assumption of spherical
symmetry in the kinematic models \citep{2012ApJ...754L..39A}. Finally, we note
that there is a third interesting case. If the lens models well-constrain
$M(r_{1/2})$ and stellar kinematics are available, then these can be used to
probe cosmological models (see \S\ref{sec:theory}). We explore this in more
detail in a separate publication. 

\section{Conclusions}\label{sec:conclusions}

We have introduced a new gravitational len modelling tool---\Glass---and used
it to test the recovery of the mass profile and shape of mock strong lensing
galaxies. We have also shown that with the addition of stellar mass
constraints, one can better constrain the inner profile of galaxies,
particularly in the case of doubles. Furthermore, by we have demonstrated our
sensitivity to the estimates of the velocity dispersion. Improved measurements
of the dispersion and, in particular, the velocity anisotropy could be used to
greatly enhance our lens models.
...
particle distributions for the mock galaxies. This work has been partially
funded by \hilight{if we want..}
...
%\caption{\textbf{Left:} Test for convergence of the predicted time delay as $\Rmap$ is increased and
%$\Rpix=45$ is held constant. After about $\Rmap=20$ most of the asymmetric mass is in the map.
%\textbf{Right:} Test for convergence of the predicted image positions $\theta$ as $\Rpix$
%changes and $\Rmap=20$ arcsec is held constant.}
