...
The $D$ factors are angular diameter distances, as in the previous section.
The lens can then be thought of as a projected surface density
$\Sigma$ which diverts the path of a photon instantaneously through
the bending angle $\vec\alpha$.  
...
dimensionless number that depends on cosmology.
...
\begin{equation}
\kappa(\vec\theta) \equiv \frac{4\pi GD_L}{c^2} \Sigma (\vec\theta)
\end{equation}

\begin{equation}
\kappa \sim \frac{z_L\,\Sigma}{10\rm\,kg\,m^{-2}}
\end{equation}
...
, with all units
and cosmology included, we have 
%
\begin{eqnarray}
