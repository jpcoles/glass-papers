...
With this coming explosion in lens data, it is timely to re-asses the impact of model degeneracies on quantities of interest like the circularly averaged density profile of a lens or its shape. Some significant work has already proceeded in this direction in the literature. XXX found that YYY. However, most of these studies employed simplified models that assume a specific analytic form for the lens and/or connect the shape of the underlying dark matter halo to its parent galaxy. Notable exceptions are XXX. As the quality and quantity of data improve, we can afford relax some of our historical model assumptions and attempt to actually {\it image} the distribution of mass in lensing galaxies and clusters. We refer to this as `non-parametric' mass modelling, where `non-' means that we deliberately employ far more model parameters than data constraints. Searching through our model parameter space, we can then explore model degeneracies and determine which data best break them.
...
%that builds on an earlier code \PixeLens\ \citep{1997MNRAS.292..148S,2008ApJ...679...17C}. Like \PixeLens, we

In this paper, we present a new non-parametric strong lens modelling framework \Glass\ that uses an adaptive grid of mass pixels to model the lens, searching through many models consistent with the data to marginalise over model uncertainties. \Glass\ shares some aspects with an earlier code \PixeLens\ \citep{1997MNRAS.292..148S,2008ApJ...679...17C}, but has important new functionality:
